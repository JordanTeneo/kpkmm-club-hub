...
export function MomentGallery({ photos, events }: { photos: Photo[]; events: Outing[] }) {
  const groups = albums(photos);
  ...
  return <><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,260px),1fr))", gap: 24 }}>{groups.map((album, n) => <button key={album.id} onClick={() => { setSelected(n); setIndex(0); dialog.current?.showModal(); }} style={{ padding: 0, textAlign: "left", color: "inherit", background: "#f5e7ca", border: "1px solid #c9b690", borderRadius: 16, overflow: "hidden", cursor: "pointer" }} aria-label={"Open " + album.photos[0].caption + ", " + album.photos.length + " photos"}><Image src={album.photos[0].url} alt={album.photos[0].caption} width={720} height={480} unoptimized style={{ width: "100%", height: 260, objectFit: "cover" }} /><span style={{ display: "grid", gap: 6, padding: 18 }}><strong>{album.photos[0].caption}</strong><span>{album.photos.length} photo{album.photos.length === 1 ? "" : "s"} · View moment →</span><small>{events.find(e => e.id === album.photos[0].eventId)?.title}</small></span></button>)}</div><dialog ref={dialog} aria-label="Shared moment photos" onClick={event => { if (event.target === dialog.current) dialog.current.close(); }} onKeyDown={event => { if (!group) return; if (event.key === "ArrowRight") setIndex(i => (i + 1) % group.photos.length); if (event.key === "ArrowLeft") setIndex(i => (i + group.photos.length - 1) % group.photos.length); }} style={{ width: "min(960px,94vw)", maxHeight: "92vh", padding: 20, border: 0, borderRadius: 16, background: "#fbf1d9", color: "#30291f" }}><style>{`dialog::backdrop{background:rgba(20,17,12,.8)}`}</style><button autoFocus onClick={() => dialog.current?.close()} style={{ float: "right", padding: 10 }}>Close ✕</button><h3>{group?.photos[0].caption}</h3>{photo && <Image src={photo.url} alt={photo.caption + " photo " + (index + 1)} width={1600} height={1100} unoptimized style={{ width: "100%", height: "min(60vh,620px)", objectFit: "contain" }} />}<div style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: 12 }}><button disabled={index === 0} onClick={() => setIndex(i => i - 1)}>← Previous</button><span aria-live="polite">{index + 1} / {group?.photos.length || 0}</span><button disabled={!group || index >= group.photos.length - 1} onClick={() => setIndex(i => i + 1)}>Next →</button></div><div style={{ display: "flex", gap: 8, overflowX: "auto" }}>{group?.photos.map((p, i) => <button key={p.id} aria-label={"View photo " + (i + 1)} aria-pressed={i === index} onClick={() => setIndex(i)} style={{ flexShrink: 0, padding: 3, border: i === index ? "3px solid #a64320" : "3px solid transparent" }}><Image src={p.url} alt="" width={80} height={60} unoptimized style={{ objectFit: "cover" }} /></button>)}</div></dialog></>;
}
